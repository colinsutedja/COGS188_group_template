...
class Game:
    ...
    def __init__(self):
        ...
        self.p2_wins=0
        self.total_games = 0
        self.p1_win_rate = 0.0
        self.p2_win_rate = 0.0
        self.move_computation_times = []
        self.white_total_score = []
        self.black_total_score = []
        self.i = 1
    ...
    def menu_screen(self):
        ...
        menu.add.label("ChessAI", align=pygame_menu.locals.ALIGN_CENTER, 
                    #    font=pygame_menu.font.FONT_OPEN_SANS_BOLD,s
               font_size=90, margin=(0, 50), font_color=LARGE_TEXT_COLOR)
        ...
        menu.add.selector('AI : ', [('Minimax', 1), ('Random', 2), ('montecarlo', 3)], onchange=self.set_ai)
    ...
    def determine_move(self):
        ...
        elif self.p2_name == "Random":
            start = time.time()
            self.ai_move.put(AI.random_move(self.board))
            end = time.time()
        else:
            start = time.time()
            self.ai_move.put(AI.monte_carlo(self.board))
            end = time.time()

        self.move_computation_times.append(end-start)
        ...
        while True:
            ...
            if self.board.gameover:
                ...
                if self.board.gameover[0] == "Insufficient Material" or self.board.gameover[0] == "Stalemate":
                    self.white_total_score.append(self.board.whiteScore)
                    self.black_total_score.append(self.board.blackScore)
                    print("white", self.board.whiteScore)
                    print("black", self.board.blackScore)
                ...
                elif self.board.gameover[1] == self.board.player:
                    print(f"GAME OVER: {self.p1_name} wins")
                    self.p1_wins += 1  # Increment Player 1's win count
                    return self.end_screen(self.board.gameover[0], self.p1_name)
                else:
                    print(f"GAME OVER: {self.p2_name} wins")
                    self.p2_wins += 1  # Increment Player 2's win count
            ...
            if self.i > self.ep:
                print("inside")
                self.p1_win_rate = self.p1_wins/ self.ep
                self.p2_win_rate = self.p2_wins/self.ep
                self.create_plots()
                exit()
...
# After the game loop, create plots
    def create_plots(self):
        # # Plot for Win Rate of Player 1 (p1)
        # plt.figure(figsize=(10, 5))
        # plt.plot(range(1, self.ep), self.p1_win_rate, marker='o', linestyle='-')
        # plt.title('Win Rate of Player 1 (p1)')
        # plt.xlabel('Games Played')
        # plt.ylabel('Win Rate')
        # plt.grid(True)
        # plt.show()
        # Plot a bar graph comparing win rates of Player 1 and Player 2
        plt.figure(figsize=(10, 5))
        players = ['Player 1', 'Player 2']
        win_rates = [self.p1_win_rate, self.p2_win_rate]
    
        plt.bar(players, win_rates, color=['blue', 'red'])
        plt.title('Win Rate Comparison Between Player 1 and Player 2')
        plt.xlabel('Players')
        plt.ylabel('Win Rate')
        plt.ylim(0, 1)  # Assuming win rates range between 0 and 1
        plt.grid(axis='y')
        plt.show()

        # Plot for Move Computation Times
        plt.figure(figsize=(10, 5))
        plt.plot(range(1, len(self.move_computation_times) + 1), self.move_computation_times, marker='o', linestyle='-')
        plt.title('Move Computation Times')
        plt.xlabel('Move Number')
        plt.ylabel('Computation Time (seconds)')
        plt.grid(True)
        plt.show()

        # Plot for Total Scores 
        plt.figure(figsize=(10, 5))
        plt.plot(range(1, len(self.white_total_score) + 1), self.white_total_score, color='blue', marker='o', linestyle='-')
        plt.plot(range(1, len(self.black_total_score) + 1), self.black_total_score, color='red', marker='o', linestyle='-')
        plt.title('Scores for Player 1 (Blue) and Player 2 (Red)')
